...
def add_question_to_template_domain(
    ...
    # Prendi version_id e domain_id dal template_domain
    td = db.query(models.TemplateDomain).filter(
        models.TemplateDomain.id == template_domain_id
    ).first()
    
    if not td:
        raise ValueError(f"TemplateDomain {template_domain_id} not found")
    
    # Genera code univoco se non fornito
    import uuid
    code = f"Q_{uuid.uuid4().hex[:8]}"
