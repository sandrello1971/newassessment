...
}> = ({ value, isNotApplicable, onChange, questionText, dataProcess, dataCategory, dataDimension }) => {  
            ...
            <input
              ...
              onKeyDown={(e) => {
                if ((e.key === 'ArrowDown' || e.key === 'ArrowUp') && e.ctrlKey) {
                  e.preventDefault();
                  e.stopPropagation();
console.log('Key pressed:', e.key, 'Current:', { dataProcess, dataCategory, dataDimension });                  
                  // Trova tutti gli slider con stessa categoria e dimensione
                  const allSliders = Array.from(
                    document.querySelectorAll('input[type="range"]')
                  ) as HTMLInputElement[];
                  
                  const currentCategory = dataCategory;
                  const currentDimension = dataDimension;
                  const currentProcess = dataProcess;
                  
                  // Filtra slider con stessa categoria e dimensione
                  const sameColumnSliders = allSliders.filter(slider => 
                    slider.getAttribute('data-category') === currentCategory &&
                    slider.getAttribute('data-dimension') === currentDimension
                  );
                  
                  // Trova indice corrente
                  const currentIndex = sameColumnSliders.findIndex(slider =>
                    slider.getAttribute('data-process') === currentProcess
                  );
                  
                  // Naviga su/giù
                  if (e.key === 'ArrowUp' && currentIndex > 0) {
                    sameColumnSliders[currentIndex - 1].focus();
console.log('Same column sliders found:', sameColumnSliders.length, 'Current index:', currentIndex);
                  } else if (e.key === 'ArrowDown' && currentIndex < sameColumnSliders.length - 1) {
                    sameColumnSliders[currentIndex + 1].focus();
                  }
                  if (e.key === 'ArrowUp') {
                    if (currentIndex > 0) {
                      allSliders[currentIndex - 1].focus();
                    }
                  } else {
...
<ScoreSelector
  ...
  dataProcess={dim.process}
  dataCategory={dim.category}
  dataDimension={dim.dimension}
/>
                            <input
                              type="text"
